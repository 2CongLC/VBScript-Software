...
Const InsDir = "Navicat Premium"
Const ZIP = "Navicat Premium.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & ZIP
Dim Mode : If IsProc = "x86" Then Mode = "navicat.exe" Else Mode = "navicat.exe"


If FSO.FileExists(Files) Then 
 TaskKill(Mode)
 Call ProcessUnzip(Files, InsDir)
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & InsDir & "\" & IsProc & "\" & Mode) Then
  WS.Run Chr(34) & Envs("%ProgramFiles%") & "\" & InsDir & "\" & IsProc & "\" &  "Setting.vbs" & Chr(34)
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & InsDir & "\" & IsProc, Mode, "Navicat Premium")
  WS.Exec(Envs("%ProgramFiles%") & "\" & InsDir & "\" & IsProc & "\" & Mode)
  End if
 If IsProc = "x86" Then FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & InsDir & "\x64" )
 If IsProc = "x64" Then FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & InsDir & "\x86" )
 Else
 If FSO.FolderExists(Root & "\Extract\" & InsDir & "\" & IsProc & "\" ) Then
  WS.Run Chr(34) & Root & "\Extract\" & InsDir & "\" & IsProc & "\" & "Setting.vbs" & Chr(34)
  Call DesktopShortCut(Root & "\Extract\" & InsDir & "\" & IsProc, Mode, "Navicat Premium")
  WS.Exec(Root & "\Extract\" & InsDir & "\" & IsProc & "\" & Mode)
  End if
 If IsProc = "x86" Then FSO.DeleteFolder(Root & "\Extract\" & InsDir &  "\x64")
 If IsProc = "x64" Then FSO.DeleteFolder(Root & "\Extract\" & InsDir &  "\x86")
 End if
