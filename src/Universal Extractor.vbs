'Coder : 2CongLC.Vn
'File Name : Universal Extractor.vbs
...
Const Source = "Universal Extractor.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "UniExtract.exe" Else ProgramExe = "UniExtract.exe"
Dim ProgramFolder : ProgramFolder = "Universal Extractor"

If FSO.FileExists(Files) Then 
 TaskKill(ProgramExe)
 Call ProcessUnzip(Files, ProgramFolder)
 If FSO.FolderExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\") Then
  WS.Run Chr(34) & Envs("%ProgramFiles%") & "\" & ProgramFolder & "\Setting.vbs" & Chr(34)
  End if
 Else
 If FSO.FolderExists(Root & "\Extract\" & ProgramFolder & "\") Then
  WS.Run Chr(34) & Root & "\Extract\" & ProgramFolder & "\Setting.vbs" & Chr(34) 
  End if
 End if
