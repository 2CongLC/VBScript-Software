...
Const Source = "WinRar.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "WinRar.exe" Else ProgramExe = "WinRar.exe"
Dim ProgramFolder :ProgramFolder = "WinRar"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe) Then
  ...
  End if
 If IsProc = "x86" Then FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\x64" )
 If IsProc = "x64" Then FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\x86" )
 Else
 If FSO.FolderExists(Root & "\Extract\" & ProgramFolder & "\" & IsProc & "\" ) Then
  ...
  End if
 If IsProc = "x86" Then FSO.DeleteFolder(Root & "\Extract\" & ProgramFolder &  "\x64")
 If IsProc = "x64" Then FSO.DeleteFolder(Root & "\Extract\" & ProgramFolder &  "\x86")
 End if
