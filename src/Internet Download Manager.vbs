'Coder : 2CongLC.Vn
'File Name : Internet Download Manager.vbs
...
Const InsDir = "Internet Download Manager"
Const ZIP = "Internet Download Manager.zip"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & InsDir & "\IDMan.exe") Then
  WS.Run Chr(34) & Envs("%ProgramFiles%") & "\" & InsDir & "\Setting.vbs" & Chr(34)
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & InsDir,"IDMan.exe","Internet Download Manager")
  WS.Exec(Envs("%ProgramFiles%") & "\" & InsDir & "\IDMan.exe")
  End if
 Else
 If FSO.FolderExists(Root & "\Extract\" & InsDir) Then
  WS.Run Chr(34) & Root & "\Extract\" & InsDir & "\Setting.vbs" & Chr(34)
  Call DesktopShortCut(Root & "\Extract\" & InsDir,"IDMan.exe","Internet Download Manager")
  WS.Exec(Root & "\Extract\" & InsDir & "\IDMan.exe")
  End if
 End if
