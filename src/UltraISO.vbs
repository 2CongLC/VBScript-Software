...
Const Source = "UltraISO.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "UltraISO.exe" Else ProgramExe = "UltraISO.exe"
Dim ProgramFolder : ProgramFolder = "UltraISO"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & ProgramExe) Then
  ...
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\", ProgramExe, "UltraISO")
