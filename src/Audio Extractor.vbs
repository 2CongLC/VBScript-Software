...
Const InsDir = "pazera-software\Audio_Extractor_32"
Const ZIP = "Audio Extractor.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & ZIP
Dim Mode : If IsProc = "x86" Then Mode = "AudioExtractor.exe" Else Mode = "AudioExtractor.exe"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & InsDir & "\" & Mode) Then
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & InsDir, Mode, "Audio Extractor")
  WS.Exec(Envs("%ProgramFiles%") & "\" & InsDir & "\" & Mode)
 End if
Else
 If FSO.FolderExists(Root & "\Extract\" & InsDir & "\" ) Then
  Call DesktopShortCut(Root & "\Extract\" & InsDir, Mode, "Audio Extractor")
