...
Const Source = "EVKey.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "EVKey32.exe" Else ProgramExe = "EVKey64.exe"
Dim ProgramFolder : ProgramFolder = "EVKey"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & ProgramExe) Then
  ...
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\", ProgramExe, "EVKey")
 ...
 If FSO.FolderExists(Root & "\Extract\" & ProgramFolder & "\"  ) Then
  ...
  Call DesktopShortCut(Root & "\Extract\" & ProgramFolder & "\", ProgramExe, "EVKey")
