...
Const Source = "Notepad++.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "notepad++.exe" Else ProgramExe = "notepad++.exe"
Dim ProgramFolder : ProgramFolder = "Notepad++"

If FSO.FileExists(Files) Then 
 TaskKill(ProgramExe)
 Call ProcessUnzip(Files, ProgramFolder)
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe) Then
  WS.Run Chr(34) & Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" &  "Setting.vbs" & Chr(34)
  'PinToTaskbar(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe)
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc, ProgramExe, "Notepad++")
  WS.Exec(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe)
  End if
 If IsProc = "x86" Then 
  FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\x64" )
 Else 
  FSO.DeleteFolder(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\x86" )
 End if
Else
 If FSO.FolderExists(Root & "\Extract\" & ProgramFolder & "\" & IsProc & "\" ) Then
  WS.Run Chr(34) & Root & "\Extract\" & ProgramFolder & "\" & IsProc & "\" & "Setting.vbs" & Chr(34)

  WS.Exec(Root & "\Extract\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe)
  End if
 If IsProc = "x86" Then 
  FSO.DeleteFolder(Root & "\Extract\" & ProgramFolder &  "\x64")
 Else 
  FSO.DeleteFolder(Root & "\Extract\" & ProgramFolder &  "\x86")
 End if 
End if
