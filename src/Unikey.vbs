...
Const Source = "Unikey.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & Source
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "UniKeyNT32.exe" Else ProgramExe = "UniKeyNT64.exe"
Dim ProgramFolder : ProgramFolder = "Unikey"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & ProgramExe) Then
  WS.Run Chr(34) & Envs("%ProgramFiles%") & "\" & ProgramFolder & "\fonts\install.cmd" & Chr(34)
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\", ProgramExe, "Unikey")
  WS.Exec(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" &  ProgramExe)
  End if
 Else
 If FSO.FolderExists(Root & "\Extract\" & ProgramFolder & "\"  ) Then
  WS.Run Chr(34) & Root & "\Extract\" & ProgramFolder & "\fonts\install.cmd" & Chr(34)
  Call DesktopShortCut(Root & "\Extract\" & ProgramFolder & "\", ProgramExe, "Unikey")
