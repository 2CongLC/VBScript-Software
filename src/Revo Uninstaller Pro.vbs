'Coder : 2CongLC.Vn
'File Name : Revo Uninstaller Pro.vbs
'Copyright © 2017 By 2CongLC.Vn | All Rights Reserved.
'Modified Date : 31.08.2020
'Version : 4.3.3
...
Const ZIP = "Revo Uninstaller Pro.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & ZIP
Dim ProgramExe : If IsProc = "x86" Then ProgramExe = "RevoUninPro.exe" Else ProgramExe = "RevoUninPro.exe"
Dim ProgramFolder : ProgramFolder = "Revo Uninstaller Pro"
...
If FSO.FileExists(Files) Then 
 ...
 If FSO.FileExists(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc & "\" & ProgramExe) Then
  ...
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & ProgramFolder & "\" & IsProc, ProgramExe, "Revo Uninstaller Pro")
