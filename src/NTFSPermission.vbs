...
Const InsDir = "NTFSPermission"
Const ZIP = "NTFSPermission.zip"

Dim Root : Root = FSO.GetParentFolderName(WScript.ScriptFullName)
Dim Files : Files = Root & "\Files\" & ZIP
Dim Mode : If IsProc = "x86" Then Mode = "NPT86.exe" Else Mode = "NPT64.exe"
...
If FSO.FileExists(Files) Then 
 ...
 ' PinToTaskbar(Envs("%ProgramFiles%") & "\" & InsDir & "\" & Mode)
  Call DesktopShortCut(Envs("%ProgramFiles%") & "\" & InsDir, Mode, "NTFS Permission")
...
Else
 ...
 ' WS.Run Chr(34) & Root & "\Extract\" & InsDir & "\" & "Setting.vbs" & Chr(34)
  'PinToTaskbar(Root & "\Extract\" & InsDir & "\" & Mode)
  Call DesktopShortCut(Root & "\Extract\" & InsDir, Mode, "NTFS Permission")
